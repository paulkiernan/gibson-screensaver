...
I've run this host on Windows and it works, and CI builds and tests the binary
on every push.

One thing worth knowing: the renderer is shared with every other host
and gets exercised constantly, but the Windows-specific plumbing - the `/s`,
`/p <hwnd>` and `/c` protocol paths, per-monitor fullscreen window creation,
and the input handling that dismisses the saver - has had far less mileage than
the macOS and web paths. If something misbehaves there, it is more likely to be
in this file's code than in the renderer, so please open an issue with the
argument the system passed.
